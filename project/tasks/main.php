...
/* تنسيق عنصري اليمين واليسار في الشريط */
.navbar a {
  float: left; /* تحديد تمركز العناصر على الجوانب */
  display: block;
  color: white;
  text-align: center;
  padding: 14px 16px;
...
<div class="container">
    <?php foreach($users as $user): ?>
        <div class="user">
            ...
            <div class="task">Deleted Task <?php echo $user['deletet_task']; ?></div>
            <div class="task">Done Task <?php echo $user['done_task']; ?></div>
            <div class="task">Not Done <?php echo $user['not_done_yet']; ?></div>
        </div>
    <?php endforeach; ?>
</div>
